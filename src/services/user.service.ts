...
import { GetUserOutput, SigninInput, SigninOutput, UpdateUserPasswordInput } from "../schema/user/signin.user.js";
...
class UserService {
  async updateUserPassword(input: UpdateUserPasswordInput): Promise<Boolean> {
    await UserModel.findOneAndUpdate({ _id: input.id }, { password: input.password }).exec()
    return true;
  }
  ...
  async signin(input: SigninInput): Promise<SigninOutput> {
    ...
    const out: SigninOutput = { refreshToken, accessToken, email: user.email, messages: message };
  ...
  //   if (input) {
  //     const output: VerifyTokenOutput = {
  //       refreshToken: "",
  //       accessToken: "",
  //     }
  //     try {
  //       const decoded = jwt.decode(input.toString());
  //       output.isExpired = jwt.isTokenExpired(decoded.exp)
